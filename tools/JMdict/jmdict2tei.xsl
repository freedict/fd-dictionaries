...
<xsl:variable name="targetlang" select="'ger'"/>	
...
<text><body><xsl:text>&#xa;</xsl:text>
	<xsl:for-each select="entry">
		<xsl:if test="sense/gloss[@xml:lang = $targetlang]">	
...
<xsl:template match="sense">
	<xsl:if test="gloss[@xml:lang = $targetlang]">
...
<xsl:template match="gloss">
	<xsl:if test="@xml:lang = $targetlang">
